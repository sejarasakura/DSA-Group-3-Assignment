...
public class ${name} implements Filter {
	...
	if (debug) log("${name}:DoBeforeProcessing");
	...
	*/
	...
	throws IOException, ServletException {
	if (debug) log("${name}:DoAfterProcessing");
...
<#if java15style??>
	...
	if (filterConfig == null) return ("${name}()");
